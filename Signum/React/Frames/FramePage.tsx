...
export default function FramePage() {
  ...
  if (state && state.pack.entity.Type != ti.name) state = undefined;

  if (state && id != null && state.pack.entity.id != id) state = undefined;

  useTitle(getToString(state?.pack.entity) ?? "", [state?.pack.entity]);

  useLooseChanges(
    state && !state.executing
      ? { entity: state.pack.entity, lastEntity: state.lastEntity }
      : undefined
  );
  ...
  React.useEffect(() => {
    ...
    if (currentEntity && currentEntity.Type == type && currentEntity.id == id) {
      if (
        stateRef.current?.viewName !=
        QueryString.parse(location.search).viewName
      ) {
        ...
        loadComponent(a.pack!).then((view) => {
          ...
          return setPack(a.pack!, view, a.createNew).then(() => {
            if (id == null && a.pack!.entity.id != null) {
              //Constructor returns saved entity
              AppContext.navigate(Navigator.navigateRoute(a.pack!.entity), {
                replace: true
              });
            }
          });
  ...
  async function loadComponent(
    ...
    if (forceViewName instanceof Navigator.ViewPromise) {
      ...
      const viewName =
        ...
        QueryString.parse(location.search).viewName ??
        undefined;
      const getComponent = await Navigator.getViewPromise(pack.entity, viewName)
        .promise;
  ...
  > {
    ...
    if (id) {
      ...
      if (cn != null && typeof cn == "string") {
        const oi = Operations.operationInfos(ti).single(
          (a) =>
            a.operationType == "Constructor" &&
            a.key.toLowerCase().endsWith(cn.toLowerCase())
        );
        ...
        return {
          ...
          createNew: () =>
            Constructor.constructPack(ti.name) as Promise<EntityPack<Entity>>
        };
      }
    }
  }
  ...
  if (!state) {
    return <div className="normal-control">{renderTitle()}</div>;
  }
  ...
  const frame: EntityFrame = {
    ...
    onReload: (pack, reloadComponent, callback) => {
      ...
      var forcedViewName =
        typeof reloadComponent == "string" ? reloadComponent : undefined;

      var currentViewName = QueryString.parse(location.search).viewName;

      var newRoute =
        is(packEntity.entity, entity) &&
        (forcedViewName ?? currentViewName) == currentViewName
          ? null
          : packEntity.entity.isNew
          ? Navigator.createRoute(
              packEntity.entity.Type,
              forcedViewName ?? currentViewName
            )
          : Navigator.navigateRoute(
              packEntity.entity,
              forcedViewName ?? currentViewName
            );
      ...
      if (reloadComponent) {
        setState(undefined)
          .then(() =>
            loadComponent(
              packEntity,
              reloadComponent == true ? undefined : reloadComponent
            )
          )
          .then((gc) => {
            if (mounted.current) {
              setPack(packEntity, gc).then(() => {
                if (newRoute) {
                  ...
                  else AppContext.navigate(newRoute);
                }
        ...
        setPack(packEntity, {
          ...
          if (newRoute) {
            if (replaceRoute) AppContext.navigate(newRoute, { replace: true });
            else AppContext.navigate(newRoute);
          }
    ...
    onClose: () => onClose(),
    revalidate: () =>
      validationErrors.current && validationErrors.current.forceUpdate(),
  ...
  const ctx = new TypeContext<Entity>(
    ...
    new ReadonlyBinding(entity, "framePage")
  );
  ...
  var outdated =
    !state.pack.entity.isNew &&
    (state.pack.entity.Type != type || state.pack.entity.id != id);

  return (
    <div
      className="normal-control"
      style={{ opacity: outdated ? 0.5 : undefined }}
    >
      {renderTitle()}
      <div style={state.executing == true ? { opacity: ".7" } : undefined}>
        <div className="sf-button-widget-container">
          {entityComponent.current && (
            <ButtonBar ref={buttonBar} frame={frame} pack={state.pack} />
          )}
        </div>
        <ValidationErrors
          ref={validationErrors}
          entity={state.pack.entity}
          prefix="framePage"
        />
        <WidgetEmbedded widgetContext={wc}>
          <div
            className="sf-main-control"
            data-refresh-count={state.refreshCount}
            data-main-entity={entityInfo(ctx.value)}
          >
            <ErrorBoundary>
              {state.getComponent && (
                <AutoFocus>
                  {FunctionalAdapter.withRef(state.getComponent(ctx), (c) =>
                    setComponent(c)
                  )}
                </AutoFocus>
              )}
  ...
  function renderTitle() {
    if (!state)
      return (
        <h3 className="display-6 sf-entity-title">
    ...
    return (
      <h4
        className={classes(
          ...
          settings?.stickyHeader && "sf-sticky-header"
        )}
      >
        {title && (
          <>
  ...
  React.useEffect(() => {
    if (blocker.state === "blocked") {
      let proceed = window.confirm(
        JavascriptMessage.loseCurrentChanges.niceToString()
      );
      if (proceed) {
